...
const Potentialleads = () => {
  ...
  const [filterOppStatus1, setFilterOppStatus1] = useState(
    localStorage.getItem("filterOppStatus1") || "In Progress"
  );
  const [filterOppStatus2, setFilterOppStatus2] = useState(
    localStorage.getItem("filterOppStatus2") || ""
  );
  const [filterManager, setFilterManager] = useState(
    localStorage.getItem("filterManager") || ""
  );
  const [filterAssignee, setFilterAssignee] = useState(
    localStorage.getItem("filterAssignee") || ""
  );
  ...
  const [appliedFilterStartDate, setAppliedFilterStartDate] = useState(
    localStorage.getItem("appliedFilterStartDate") || ""
  );
  const [appliedFilterEndDate, setAppliedFilterEndDate] = useState(
    localStorage.getItem("appliedFilterEndDate") || ""
  );
...
XLSX.writeFile(wb, fileName);
    ...
    try {
      ...
      if (response.status == 200) {
        const filteredLeads = response.data.filter(
          (enquiry) =>
            enquiry.adminAssign !== "admin" && enquiry.status == "opportunity"
        );
  ...
  const handleSelfAssign = async (leadid) => {
    try {
      const response = await axios.post(`${baseURL}/api/assign-admin`, {
        leadid,
      });
      ...
      setMessage(
        "An error occurred while assigning the lead. Please try again."
      );
      setTimeout(() => setMessage(""), 1000);
    }
  };
  ...
  const handleSecondaryStatusChange = (value, rowId) => {
    setData((prevData) => {
      ...
      const primaryStatus = updatedData.find(
        (row) => row.leadid == rowId
      ).opportunity_status1;
  ...
  const handleEdit = (rowId) => {
    console.log("Navigating with lead ID:", rowId); // Debugging
    if (!rowId) {
    ...
    return data.filter((item) => {
      ...
      const matchesDestination =
        ...
        (item.travel_destination &&
          item.travel_destination.toLowerCase() ==
            filterDestination.toLowerCase());
      const actualPrimaryStatus = item.opportunity_status1
        ? item.opportunity_status1.toLowerCase()
        : "In Progress";
      const matchesOppStatus1 =
        !filterOppStatus1 ||
        actualPrimaryStatus === filterOppStatus1.toLowerCase();
      const matchesOppStatus2 =
        !filterOppStatus2 ||
        (item.opportunity_status2 &&
          item.opportunity_status2.toLowerCase() ==
            filterOppStatus2.toLowerCase());
      const matchesManager =
        !filterManager ||
        (item.assign_to_manager &&
          item.assign_to_manager.toLowerCase() == filterManager.toLowerCase());
      const matchesAssignee =
        !filterAssignee ||
        (item.assignedSalesName &&
          item.assignedSalesName.toLowerCase() == filterAssignee.toLowerCase());
      ...
      { Header: "Opp Id", accessor: "leadid" },
      {
        ...
        Cell: ({ row }) => (
          <div
            ...
            onClick={() => navigate(`/a-details/${row.original.leadid}`)}
          >
            {row.original.name}
          </div>
        ),
...

          {
            ...
            Cell: ({ row }) => {
              ...
              const handleNavigate = () => {
                navigate(`/email-history/${row.original.leadid}`, { 
        ...
        ,
          {
            ...
            Cell: ({ row }) => {
              ...
              const handleChange = (e) => {
                ...
                setShowIcon(
                  newValue !== `${assignedManagerId}|${assignedManagerName}`
                );
              };
...

      {
        ...
        Cell: ({ row }) => {
          ...
          const [selectedAssociate, setSelectedAssociate] = useState(
            initialAssociateValue
          );
          const [showIcon, setShowIcon] = useState(false);

          const managerId = row.original.managerid;
          const associates = managerId
            ? associatesByManager[managerId] || []
            : [];
          ...
          const handleAssignClick = async () => {
            if (selectedAssociate) {
              ...
              await handleAssignLead(
                row.original.leadid,
                { id: associateId, name: associateName },
                row.original.status
              );
            ...
            <div className="d-flex align-items-center" style={{ fontSize: fontSize }}>
              ...
              >
                ...
                {associates.map((associate, index) => (
                  <option
                    key={index}
                    value={`${associate.id}|${associate.name}`}
                  >
                    {associate.name}
                  </option>
                ))}
  ...
  const uniqueDestinations = useMemo(() => {
    ...
    return [...new Set(normalizedDestinations)].map(
  ...
  const teamMembers = useMemo(() => {
    ...
    const selectedManager = managers.find(
      (manager) => manager.name.toLowerCase() == filterManager.toLowerCase()
    );
